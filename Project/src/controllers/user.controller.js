...
const getWatchHistory = asyncHandler(async(req,res)=>{
  const user = await User.aggregate([
    ...
    {
      $lookup:{
        from:"videos",         // It is Video (it will be in plural in data base) from video.model.js
        localField:"watchHistory",
        foreignField:"_id",
        as:"watchHistory",
        pipeline:[                  // This is for nested Pipelines
          {
            $lookup:{
              from:"users",
              localField:"owner",
          ...
          {
            $addFields:{
              owner:{             // this will make Frontend Developer Job easy because in above owner it was going as [{}] now after this {}
                $first:"$owner"
              }
            }
          }
...
console.log(jokes,"22")

export { registerUser, loginUser, logoutUser,refreshAccessToken,changeCurrentPassword,getCurrentUser,updateAccountDetails,updateUserAvatar,updateUserCoverImage,getUserChannelProfile,getWatchHistory ,jokes };
